...
public class CustomerController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service.equals("listAllCustomer")) {
            ...
            ResultSet rs = dao.getData("select * from Customers");
            request.setAttribute("data", rs);
            request.setAttribute("message", message);
            request.getRequestDispatcher("/view/customer/listAllCustomer.jsp").forward(request, response);
        }
        if (service.equals("insertCustomer")) {
            String submit = request.getParameter("submit");
            if (submit == null) {
                request.setAttribute("notification", "");
                request.getRequestDispatcher("/view/customer/insertCustomer.jsp").forward(request, response);
            } else {
        ...
        if (service.equals("deleteCustomer")) {
            String customerID = request.getParameter("customerID");
            int n = dao.deleteCustomer(customerID);
            String noti = (n > 0) ? "Delete successfully" : "Error occurs, cannot delete";
            response.sendRedirect("CustomerURL?message=" + noti);
        }
